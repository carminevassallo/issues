...
Job gitlab_git_test is not created when certain new commits are pushed.
...
Job `gitlab_git_test`, that is defined at line , is set to not be created in stage `` if developers commit changes with the following refs:
