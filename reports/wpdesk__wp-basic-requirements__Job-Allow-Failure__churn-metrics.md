...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `churn-metrics` in [.gitlab-ci.yml](https://gitlab.com/wpdesk/wp-basic-requirements/blob/master/.gitlab-ci.yml)
