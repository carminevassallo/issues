...
Failures of job audit-node do not fail the overall build.
...
Failures of job `audit-node`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `audit-node` in [.gitlab-ci.yml](https://gitlab.com/edgyemma/Postmill/blob/master/.gitlab-ci.yml)
