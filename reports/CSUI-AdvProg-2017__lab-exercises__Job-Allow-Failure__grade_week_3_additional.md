...
Failures of job grade_week_3_additional do not fail the overall build.
...
Failures of job `grade_week_3_additional`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `grade_week_3_additional` in [.gitlab-ci.yml](https://gitlab.com/CSUI-AdvProg-2017/lab-exercises/blob/master/.gitlab-ci.yml)
