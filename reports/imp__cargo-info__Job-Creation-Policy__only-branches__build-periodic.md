...
Job build-periodic is not created when certain new commits are pushed.

## Description
Developers should follow principles (or best practices) in order to properly practice Continuous Integration (CI) and Delivery (CD).
We found that [.gitlab-ci.yml](https://gitlab.com/imp/cargo-info/blob/master/.gitlab-ci.yml) contains a violation of the following CI/CD principle:
...
Job `build-periodic`, that is defined at line , is set to  be created in stage `` if developers commit changes to some branches
