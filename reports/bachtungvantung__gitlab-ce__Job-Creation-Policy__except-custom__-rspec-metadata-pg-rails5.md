...
Job -rspec-metadata-pg-rails5 is not created when certain new commits are pushed.
...
Job `-rspec-metadata-pg-rails5`, that is defined at line , is set to not be created in stage `` if developers commit changes with the following refs:
