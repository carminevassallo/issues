...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `code_quality` in [.gitlab-ci.yml](https://gitlab.com/bachtungvantung/gitlab-ce/blob/master/.gitlab-ci.yml)
