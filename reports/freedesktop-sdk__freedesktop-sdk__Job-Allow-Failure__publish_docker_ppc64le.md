...
Failures of job publish_docker_ppc64le do not fail the overall build.
...
Failures of job `publish_docker_ppc64le`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `publish_docker_ppc64le` in [.gitlab-ci.yml](https://gitlab.com/freedesktop-sdk/freedesktop-sdk/blob/master/.gitlab-ci.yml)
