...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `test-backend` in [.gitlab-ci.yml](https://gitlab.com/spare-time-demos/intime2/blob/master/.gitlab-ci.yml)
