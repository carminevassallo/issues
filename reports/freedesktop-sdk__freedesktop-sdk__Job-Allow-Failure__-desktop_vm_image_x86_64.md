...
Failures of job -desktop_vm_image_x86_64 do not fail the overall build.
...
Failures of job `-desktop_vm_image_x86_64`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `-desktop_vm_image_x86_64` in [.gitlab-ci.yml](https://gitlab.com/freedesktop-sdk/freedesktop-sdk/blob/master/.gitlab-ci.yml)
