...
Job sensiolabs-vendor-bundled-security-check is not created when certain new commits are pushed.
...
Job `sensiolabs-vendor-bundled-security-check`, that is defined at line , is set to  be created in stage `` if developers commit changes to some branches
