...
Job check_week_4_mandatory is not created when certain new commits are pushed.
...
Job `check_week_4_mandatory`, that is defined at line , is set to  be created in stage `` if developers commit changes with the following refs only:
