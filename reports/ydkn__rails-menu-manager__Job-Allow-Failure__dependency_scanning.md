...
We found that [.gitlab-ci.yml](https://gitlab.com/ydkn/rails-menu-manager/blob/master/.gitlab-ci.yml) contains a violation of the following CI/CD principle:
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `dependency_scanning` in [.gitlab-ci.yml](https://gitlab.com/ydkn/rails-menu-manager/blob/master/.gitlab-ci.yml)
