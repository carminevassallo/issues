...
Failures of job openSUSE-tumbleweed-refclocks do not fail the overall build.
...
Failures of job `openSUSE-tumbleweed-refclocks`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `openSUSE-tumbleweed-refclocks` in [.gitlab-ci.yml](https://gitlab.com/NTPsec/ntpsec/blob/master/.gitlab-ci.yml)
