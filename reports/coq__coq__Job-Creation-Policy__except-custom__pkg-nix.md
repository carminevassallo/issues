...
Job pkg-nix is not created when certain new commits are pushed.
...
Job `pkg-nix`, that is defined at line , is set to not be created in stage `` if developers commit changes with the following refs:
