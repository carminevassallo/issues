...
Failures of job debian do not fail the overall build.
...
Failures of job `debian`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `debian` in [.gitlab-ci.yml](https://gitlab.com/WzukW/oclaunch/blob/master/.gitlab-ci.yml)
