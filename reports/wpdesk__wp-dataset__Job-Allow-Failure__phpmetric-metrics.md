...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `phpmetric-metrics` in [.gitlab-ci.yml](https://gitlab.com/wpdesk/wp-dataset/blob/master/.gitlab-ci.yml)
