...
Job -flatpak_template is not created when certain new commits are pushed.
...
Job `-flatpak_template`, that is defined at line , is set to not be created in stage `` if developers commit changes with the following refs:
