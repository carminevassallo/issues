...
Failures of job aarch64-linux-android do not fail the overall build.
...
Failures of job `aarch64-linux-android`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `aarch64-linux-android` in [.gitlab-ci.yml](https://gitlab.com/susurrus/fat-rs/blob/master/.gitlab-ci.yml)
