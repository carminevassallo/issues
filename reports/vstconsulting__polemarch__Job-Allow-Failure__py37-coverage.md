...
Failures of job py37-coverage do not fail the overall build.
...
Failures of job `py37-coverage`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `py37-coverage` in [.gitlab-ci.yml](https://gitlab.com/vstconsulting/polemarch/blob/master/.gitlab-ci.yml)
