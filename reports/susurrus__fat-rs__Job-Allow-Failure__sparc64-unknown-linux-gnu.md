...
Failures of job sparc64-unknown-linux-gnu do not fail the overall build.
...
Failures of job `sparc64-unknown-linux-gnu`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `sparc64-unknown-linux-gnu` in [.gitlab-ci.yml](https://gitlab.com/susurrus/fat-rs/blob/master/.gitlab-ci.yml)
