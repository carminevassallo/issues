...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `integration-test-current-woocommerce` in [.gitlab-ci.yml](https://gitlab.com/wpdesk/wp-settings/blob/master/.gitlab-ci.yml)
