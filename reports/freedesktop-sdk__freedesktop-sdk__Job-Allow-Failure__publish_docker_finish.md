...
Failures of job publish_docker_finish do not fail the overall build.
...
Failures of job `publish_docker_finish`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `publish_docker_finish` in [.gitlab-ci.yml](https://gitlab.com/freedesktop-sdk/freedesktop-sdk/blob/master/.gitlab-ci.yml)
