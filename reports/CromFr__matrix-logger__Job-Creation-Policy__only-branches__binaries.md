...
We found that [.gitlab-ci.yml](https://gitlab.com/CromFr/matrix-logger/blob/master/.gitlab-ci.yml) contains a violation of the following CI/CD principle:
...
Job `binaries`, that is defined at line , is set to  be created in stage `` if developers commit changes to some branches
