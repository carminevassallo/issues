...
Job minimal_systemd_vm_powerpc64le is not created when certain new commits are pushed.
...
Job `minimal_systemd_vm_powerpc64le`, that is defined at line , is set to not be created in stage `` if developers commit changes with the following refs:
