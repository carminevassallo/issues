...
Failures of job docker-gnsssdr do not fail the overall build.
...
Failures of job `docker-gnsssdr`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `docker-gnsssdr` in [.gitlab-ci.yml](https://gitlab.com/gnss-sdr/gnss-sdr/blob/master/.gitlab-ci.yml)
