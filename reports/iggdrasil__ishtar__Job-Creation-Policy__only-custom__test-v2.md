...
Job test-v2 is not created when certain new commits are pushed.
...
Job `test-v2`, that is defined at line , is set to  be created in stage `` if developers commit changes with the following refs only:
