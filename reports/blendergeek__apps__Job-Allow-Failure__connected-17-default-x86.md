...
Failures of job connected-17-default-x86 do not fail the overall build.
...
Failures of job `connected-17-default-x86`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `connected-17-default-x86` in [.gitlab-ci.yml](https://gitlab.com/blendergeek/apps/blob/master/.gitlab-ci.yml)
