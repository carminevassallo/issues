...
Job update-tests-metadata is not created when certain new commits are pushed.
...
Job `update-tests-metadata`, that is defined at line , is set to  be created in stage `` if developers commit changes with the following refs only:
