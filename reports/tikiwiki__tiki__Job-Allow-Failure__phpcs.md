...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `phpcs` in [.gitlab-ci.yml](https://gitlab.com/tikiwiki/tiki/blob/master/.gitlab-ci.yml)
