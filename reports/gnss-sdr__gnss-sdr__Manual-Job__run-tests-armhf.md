...
Job run-tests-armhf is manually triggered
...
Job `run-tests-armhf`, that is defined at line  and executed in stage ``, is set to be executed manually.
...
> **Suggested Fix:** Remove ```when:manual``` from the definition of job `run-tests-armhf`.
