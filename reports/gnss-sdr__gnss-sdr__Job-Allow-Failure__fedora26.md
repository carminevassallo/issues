...
Failures of job fedora26 do not fail the overall build.
...
Failures of job `fedora26`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `fedora26` in [.gitlab-ci.yml](https://gitlab.com/gnss-sdr/gnss-sdr/blob/master/.gitlab-ci.yml)
