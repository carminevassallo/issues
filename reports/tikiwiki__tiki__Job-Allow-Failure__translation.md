...
Failures of job translation do not fail the overall build.
...
Failures of job `translation`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `translation` in [.gitlab-ci.yml](https://gitlab.com/tikiwiki/tiki/blob/master/.gitlab-ci.yml)
