...
Failures of job test-python38 do not fail the overall build.
...
Failures of job `test-python38`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `test-python38` in [.gitlab-ci.yml](https://gitlab.com/radek-sprta/mariner/blob/master/.gitlab-ci.yml)
