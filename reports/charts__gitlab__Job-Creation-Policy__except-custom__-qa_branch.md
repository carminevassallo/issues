...
Job -qa_branch is not created when certain new commits are pushed.
...
Job `-qa_branch`, that is defined at line , is set to not be created in stage `` if developers commit changes with the following refs:
