...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `stop_review` in [.gitlab-ci.yml](https://gitlab.com/SoHigh/Nergigante/blob/master/.gitlab-ci.yml)
