...
Failures of job composer-lock do not fail the overall build.
...
Failures of job `composer-lock`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `composer-lock` in [.gitlab-ci.yml](https://gitlab.com/tikiwiki/tiki/blob/master/.gitlab-ci.yml)
