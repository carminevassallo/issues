...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `dast` in [.gitlab-ci.yml](https://gitlab.com/SoHigh/Nergigante/blob/master/.gitlab-ci.yml)
