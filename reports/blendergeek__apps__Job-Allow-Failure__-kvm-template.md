...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `-kvm-template` in [.gitlab-ci.yml](https://gitlab.com/blendergeek/apps/blob/master/.gitlab-ci.yml)
