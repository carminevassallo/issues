...
Job docker-pybombs-gnsssdr is not created when certain new commits are pushed.
...
Job `docker-pybombs-gnsssdr`, that is defined at line , is set to  be created in stage `` if developers commit changes with the following refs only:
