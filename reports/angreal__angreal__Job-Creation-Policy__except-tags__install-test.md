...
Job install-test is not created when certain new commits are pushed.
...
Job `install-test`, that is defined at line , is set to not be created in stage `` if developers commit changes with some tags
