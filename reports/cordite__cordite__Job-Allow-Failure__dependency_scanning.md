...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `dependency_scanning` in [.gitlab-ci.yml](https://gitlab.com/cordite/cordite/blob/master/.gitlab-ci.yml)
