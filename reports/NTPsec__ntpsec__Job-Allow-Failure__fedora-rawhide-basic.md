...
Failures of job fedora-rawhide-basic do not fail the overall build.
...
Failures of job `fedora-rawhide-basic`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `fedora-rawhide-basic` in [.gitlab-ci.yml](https://gitlab.com/NTPsec/ntpsec/blob/master/.gitlab-ci.yml)
