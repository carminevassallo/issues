...
Job alpine-basic is retried multiple times.
...
Job `alpine-basic`, that is defined at line  and executed in stage ``, is set to be retried  times. 
...
> **Suggested Fix:** Remove ```retry: ``` from the definition of job `alpine-basic` or consider adding the option ```when``` to specify failures cases (e.g., ```runner_system_failure```) when the job can be retried without hiding flaky tests.
