...
Failures of job deploy-push-docker-gitlab-image do not fail the overall build.
...
Failures of job `deploy-push-docker-gitlab-image`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `deploy-push-docker-gitlab-image` in [.gitlab-ci.yml](https://gitlab.com/spare-time-demos/intime2/blob/master/.gitlab-ci.yml)
