...
Failures of job publish_release do not fail the overall build.
...
Failures of job `publish_release`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `publish_release` in [.gitlab-ci.yml](https://gitlab.com/vstconsulting/polemarch/blob/master/.gitlab-ci.yml)
