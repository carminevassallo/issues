...
Job deploy-production-tf is manually triggered
...
Job `deploy-production-tf`, that is defined at line  and executed in stage ``, is set to be executed manually.
...
> **Suggested Fix:** Remove ```when:manual``` from the definition of job `deploy-production-tf`.
