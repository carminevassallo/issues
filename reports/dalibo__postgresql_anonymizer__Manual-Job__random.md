...
Job random is manually triggered
...
Job `random`, that is defined at line  and executed in stage ``, is set to be executed manually.
...
> **Suggested Fix:** Remove ```when:manual``` from the definition of job `random`.
