...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `code_quality` in [.gitlab-ci.yml](https://gitlab.com/dennismaxjung/vscode-dotnet-auto-attach/blob/master/.gitlab-ci.yml)
