...
Job -UI--Run-As-Preview is manually triggered
...
Job `-UI--Run-As-Preview`, that is defined at line  and executed in stage ``, is set to be executed manually.
...
> **Suggested Fix:** Remove ```when:manual``` from the definition of job `-UI--Run-As-Preview`.
