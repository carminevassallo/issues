...
Job review_build_job is manually triggered
...
Job `review_build_job`, that is defined at line  and executed in stage ``, is set to be executed manually.
...
> **Suggested Fix:** Remove ```when:manual``` from the definition of job `review_build_job`.
