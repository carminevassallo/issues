...
Failures of job ubuntu-devel-refclocks do not fail the overall build.
...
Failures of job `ubuntu-devel-refclocks`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `ubuntu-devel-refclocks` in [.gitlab-ci.yml](https://gitlab.com/NTPsec/ntpsec/blob/master/.gitlab-ci.yml)
