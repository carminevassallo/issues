...
Failures of job app_powerpc64le do not fail the overall build.
...
Failures of job `app_powerpc64le`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `app_powerpc64le` in [.gitlab-ci.yml](https://gitlab.com/freedesktop-sdk/freedesktop-sdk/blob/master/.gitlab-ci.yml)
