...
We found that [.gitlab-ci.yml](https://gitlab.com/gioxa/fake_arch/blob/master/.gitlab-ci.yml) contains a violation of the following CI/CD principle:
...
Job `deploy`, that is defined at line , is set to not be created in stage `` if developers commit changes to some branches
