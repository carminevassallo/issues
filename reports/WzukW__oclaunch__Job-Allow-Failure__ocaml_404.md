...
Failures of job ocaml_404 do not fail the overall build.
...
Failures of job `ocaml_404`, that is defined at line  and executed in stage ``, are set to not fail the overall build.
...
> **Suggested Fix:** Modify ```allow_failure: true``` with ```allow_failure: false``` for job `ocaml_404` in [.gitlab-ci.yml](https://gitlab.com/WzukW/oclaunch/blob/master/.gitlab-ci.yml)
